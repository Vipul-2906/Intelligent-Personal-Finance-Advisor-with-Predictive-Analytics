...
const ui = (function(){
  ...
  function safeUserId(){
    const u = getUser();
    const id = u ? (u.id || u.user_id || u.userId) : null;
    if(!id){ showToast('Session expired — please login again','error'); window.location.href='/'; }
    return id;
  }
  ...
  async function addTransaction(){
    const userId = safeUserId(); if(!userId){ showToast('Please login','error'); window.location.href='/'; return; }
    ...
    const date = document.getElementById('txnDate')?.value || document.getElementById('quickDate')?.value || new Date().toISOString().slice(0,10);
  ...
  async function updateDashboard(){
    const userId = safeUserId(); if(!userId) { window.location.href='/'; return; }

    const userNameEl = document.getElementById('userName');
    if(userNameEl) userNameEl.textContent = (localStorage.getItem('fa_user_name') || 'User');

    // Summary cards are owned by loadDashboard() via /dashboard_summary — do not write here.
  ...
  async function pageInit(){
    ...
    if(path.includes('dashboard'))  { await updateDashboard(); await setupDashboardUI(); }
    if(path.includes('transaction')) await renderTxns();
